...
WORKDIR /src
COPY ["LabProject/LabProject.csproj", "LabProject/"]
RUN dotnet restore "./LabProject/LabProject.csproj"
COPY . .
WORKDIR "/src/LabProject"
RUN dotnet build "./LabProject.csproj" -c $BUILD_CONFIGURATION -o /app/build

# Ten etap służy do publikowania projektu usługi do skopiowania do etapu końcowego
FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "./LabProject.csproj" -c $BUILD_CONFIGURATION -o /app/publish /p:UseAppHost=false
